...
export default async function CategoryDetailsPage({
  ...
  searchParams?: CategoriesSearchParams | Promise<CategoriesSearchParams>;
}) {
  ...
  const selectedCategory = categories.find(
    (category) => category.id === resolvedParams.id,
  );
  ...
  try {
    const tokens = [
      ...
      buildToken("category_is", selectedCategory.attributes.name),
    ];
  ...
  return (
    <Container size="xl" py="xl">
      <ScopedTransactionsPanel
        title={`Category: ${selectedCategory.attributes.name}`}
        ...
        limit={limit}
        basePath={`/categories/${selectedCategory.id}`}
